...
__global__ void contrast(uchar* R, uchar* G, uchar* B, float fc) {
	...
	int gId = row_offset + block_offset + threadId_inside;
	if (fc * R[gId] > 255) { R[gId] = 255; }
	else { R[gId] = fc * R[gId]; }
	if (fc * G[gId] > 255) { G[gId] = 255; }
	else { G[gId] = fc * G[gId]; }
	if (fc * B[gId] > 255) { B[gId] = 255; }
	else { B[gId] = fc * B[gId]; }
}
...
__global__ void brightness(uchar* R, uchar* G, uchar* B, float fb) {
	...
	int gId = row_offset + block_offset + threadId_inside;
	if (fb >= 0) {
		if (fb + R[gId] > 255) { R[gId] = 255; }
		else { R[gId] = fb + R[gId]; }
		if (fb + G[gId] > 255) { G[gId] = 255; }
		else { G[gId] = fb + G[gId]; }
		if (fb + B[gId] > 255) { B[gId] = 255; }
		else { B[gId] = fb + B[gId]; }
	}

	if (fb < 0) {
		if (fb + R[gId] < 0) { R[gId] = 0; }
		else { R[gId] = fb + R[gId]; }
		if (fb + G[gId] < 0) { G[gId] = 0; }
		else { G[gId] = fb + G[gId]; }
		if (fb + B[gId] < 0) { B[gId] = 0; }
		else { B[gId] = fb + B[gId]; }
	}
}
